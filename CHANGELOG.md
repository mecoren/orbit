...
- **放调优（同日两轮用户反馈）**：①"框太小挤、行标错位"——统计容器 max-w-3xl→5xl；双端周一/周三/周五行标由「首尾均分」改为与网格行同构的 7 槽精确对位（旧法"周三"中心落在周四行中心，格小错位明显）；移动端格 12→14dp。②"占满框框"——桌面格宽改动态撑满（测量目标=热力图块外层 w-full 容器 + clientWidth 直读/窗口 resize 双通道，规避 overflow 容器 ResizeObserver 首帧窄值上报与 minWidth 反撑自引用；floor 取整防差 1px 溢出），窄窗钳 12px 下限走横滚；实机断言 1280/1200/700 三档视口：格 14/13/12px、年份栏右缘贴卡片右缘、行标 356.5/386.5/416.5 逐像素对齐、宽屏无溢出窄屏横滚正常。
