...
P1 体验能力包补记（#8/#9/#10/#11/#12/#13，8/26 落地未记账）+ P2#19 冒烟接入 CI + **我的一天（My Day）三端落地**（07 报告新增 #23）+ 标题栏窗口控制键重制 + **提醒功能三端升级**（推迟操作 / 后台闹钟 / 灵动岛类别）+ **日历视图三端升级：格内待办长条 + 联网节假日**（07 报告新增 #24）+ **回收站三端落地**（删除的任务可恢复，保留时间可配）。
...
- **回收站三端落地（用户需求：删除的待办进回收站，保留时间可参考优秀开源项目）**：
  ...
  - **数据层 `trash_api`（orbit-core）**：在既有软删墓碑行之上新增
    ...
    边界决策见 **adr/0005**）
  - **桌面端**：`/trash` 路由页（行含保留期倒计时徽标，右键恢复/彻底删除，
    顶部清空确认）+ 侧栏快捷区「回收站」固定行（折叠态图标同款）+ 标题栏/
    命令面板入口 + 设置新增「待办」分类（保留档位 pill 选择）+
    `trash_scheduler` 60s tick 守护（启动首轮补清）+ 三处删除确认文案更新
    （「之后将移入回收站」）
