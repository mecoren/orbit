...
if(WIN32)
    ...
    # Capture the script path at file scope: inside a function,
    # CMAKE_CURRENT_LIST_DIR resolves to the *caller's* directory, so it
    # cannot be used there to locate this file.
    set(CARGOKIT_RESOLVE_SYMLINKS_PSSCRIPT "${CMAKE_CURRENT_LIST_DIR}/resolve_symlinks.ps1")
    execute_process(COMMAND powershell -ExecutionPolicy Bypass -File "${CARGOKIT_RESOLVE_SYMLINKS_PSSCRIPT}" "${cargokit_cmake_root}" OUTPUT_VARIABLE cargokit_cmake_root OUTPUT_STRIP_TRAILING_WHITESPACE)
endif()
...
function(apply_cargokit target manifest_dir lib_name any_symbol_name)
    ...
    if(WIN32)
        # On Windows the default cargo build dir
        # (mobile/build/windows/x64/plugins/rust_lib_wait_home/cargokit_build)
        # is so deep that openssl-src cannot extract/lay out the OpenSSL source
        # tree (paths exceed MAX_PATH), causing `Configure` to report missing
        # source files and the Rust build to fail. Redirect the cargo/openssl
        # build dir to a short path so extraction succeeds. The .dll output dir
        # (CARGOKIT_OUTPUT_DIR) is left unchanged because the .dll path itself
        # is short and MSBuild expects the .dll at its original location.
        set(CARGOKIT_TEMP_DIR "C:/ckb")
    else()
    ...
    # Resolve symlinks in the source directory before joining the relative
    # manifest_dir. On Windows CMake's REALPATH does not follow directory
    # symlinks, and the plugin is referenced through
    # .plugin_symlinks/rust_lib_wait_home -> <real>/rust_builder. A naive join of
    # CMAKE_CURRENT_SOURCE_DIR/../../rust_frbbindings therefore resolves to
    # .plugin_symlinks/rust_frbbindings (which does not exist) instead of the
    # real mobile/rust_frbbindings, so the Rust crate's Cargo.toml cannot be
    # found and the build aborts. Resolving to the real path fixes it.
    if(WIN32)
        execute_process(COMMAND powershell -ExecutionPolicy Bypass -File "${CARGOKIT_RESOLVE_SYMLINKS_PSSCRIPT}" "${CMAKE_CURRENT_SOURCE_DIR}" OUTPUT_VARIABLE CARGOKIT_MANIFEST_SOURCE_DIR OUTPUT_STRIP_TRAILING_WHITESPACE)
    else()
        get_filename_component(CARGOKIT_MANIFEST_SOURCE_DIR "${CMAKE_CURRENT_SOURCE_DIR}" REALPATH)
    endif()

    set(CARGOKIT_ENV
        "CARGOKIT_CMAKE=${CMAKE_COMMAND}"
        "CARGOKIT_CONFIGURATION=$<CONFIG>"
        "CARGOKIT_MANIFEST_DIR=${CARGOKIT_MANIFEST_SOURCE_DIR}/${manifest_dir}"
