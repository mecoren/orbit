...
- **超时语义 总超时→读超时（S4 超时半项，`d3fa8e0`）**：reqwest `timeout` 是「连接开始到响应体读完」的固定总预算，家庭下行 5-30Mbps 时 50MB 附件下载全程超 30s 即被掐断；改 `read_timeout`——每次读操作单独计时、读到一块即重置，只断停滞不断慢速。定向测试两例：慢而在动必须成功（改回 `.timeout()` 即红的回归防线）+ 真停滞必须仍掐断。审查报告 §四同步勘误三处失实记载（S4「批 B 已缓解」不存在、S18「conflict_count 恒 0」已被 F9 修掉、S23 reqwest 池实为 Client 实例级非进程级）。
- **S4 大附件 multipart 分片全量收口（三连）**：① 确定性加密 nonce（`6a711d6`）——`SHA256(data_key ‖ SHA256(compressed)hex ‖ len_be)[0..12]` 派生（SIV 语义），同 (key, 明文) 必产同密文（断点续传前提）；不同明文 nonce 碰撞等价 SHA-256 碰撞，data_key 参与哈希防构造碰撞对；format version 0x01 不变、新旧密文互通。② S3 原生 multipart（`9f2e1d3`）——8MiB 阈值/5MiB 片，Create → 逐片 UploadPart → Complete 服务端拼装、产物落原路径读者无感知；每片独立请求 + per-request 120s 总超时（覆盖发送阶段，350kbps 慢速上行单片可完成）；分片级重试独立三档不叠加放大，失败重传单片而非整文件。③ WebDAV 自造分片协议（`92c78f2`）——`assets_parts/{hash}/head.json` 明文清单 + 逐片 .bin；断点续传（HEAD 探测已存片、大小相等即跳过）；清单最后写（「完整」信号，半成品目录不可读）；读侧三段回退 + 拼装后大小/sha256 双校验；rekey 对账（分片就位后清旧单对象防读侧命中旧 Key 密文）；asset_exists/list_assets 分片感知（差集不空跑、首传三分叉不误放行）。慢速下行 / 慢速上行 / 断网中断三重失败模式全解，S4 移出遗留清单（真机大附件实测待验收）。
- 定向测试 19 项新增（附件空列表三分叉 4 + pull 真值表 4 + 附件差集过滤 3 + 探针 1 + merge 冲突计数 2 + parent_url 5）+ S4 三连 22 项（确定性 nonce 6 + S3 协议 4 + WebDAV 协议 12）；全链回归：cargo 全绿 / clippy 基线持平 / fmt 通过（tsc / vitest / e2e / flutter 未涉及 UI 层零改动）。报告：docs/同步功能全面审查与优化报告-2026-09-14.md。
