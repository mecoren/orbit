...
### 品牌图标换版（用户提供源图重构，三端全族）

- **新构图**：用户提供手绘源图（`scripts/PixPin_2026-09-07_20-16-46.png`，白底蓝主体 364×325）——左上月牙形行星 + 左下扫至右上的变宽轨道弧（末端卫星球）+ 中部小彗星，「卫星绕行星」直扣 Orbit/循迹语义。源图经清洗（去截图角标/白底）→ cv2 轮廓提取 → RDP 简化 + Chaikin 平滑去抗锯齿抖动 → 归一化形状数据固化 `scripts/icon_shapes.json`（整体 IoU 0.89，差异 100% 在 ±4px 边缘带内，无结构性失真）。
- **渲染**：保留品牌深空色 squircle 底（#1A1A21，与启动屏 launch_bg 同源），主体蓝 #3974F7；1024 母版 LANCZOS 出全族。`generate_icons.py` 重写渲染管线 + 新自检（四角透明/底色/卫星球中心色/通知纯白），数据驱动（形状 JSON 与脚本分离）。
- **覆盖**：桌面 Tauri 全族（PNG/ico/icns/Store Square）+ 关于页 `app-icon.png`；Android 五密度 `ic_launcher.png` + 通知剪影 `ic_stat_orbit.png`（24px 结构验证：月牙+弧+卫星球可辨）+ 启动屏 `launch_image.png`（全出血无圆角版）。v1 渐变椭圆轨道设计由本构图替换。
- 小尺寸结构保留经连通域验证：48px 四部件完整、32px 彗星并入弧线成两部件、16px 单色块仍含月牙+弧轮廓。
