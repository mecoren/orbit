...
- **超时语义 总超时→读超时（S4 超时半项，`d3fa8e0`）**：reqwest `timeout` 是「连接开始到响应体读完」的固定总预算，家庭下行 5-30Mbps 时 50MB 附件下载全程超 30s 即被掐断；改 `read_timeout`——每次读操作单独计时、读到一块即重置，只断停滞不断慢速。语义边界（reqwest 0.12.28 源码核实）：只覆盖响应体读取阶段，PUT 上传大请求体仍无超时保护，S4 multipart 分片维持推迟（触发器不变）。定向测试两例：慢而在动必须成功（改回 `.timeout()` 即红的回归防线）+ 真停滞必须仍掐断；orbit-core 511 全绿。审查报告 §四同步勘误三处失实记载（S4「批 B 已缓解」不存在、S18「conflict_count 恒 0」已被 F9 修掉、S23 reqwest 池实为 Client 实例级非进程级）。
