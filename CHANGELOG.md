...
- **放调优（同日用户反馈"框太小挤、行标错位"）**：桌面格宽 10→12px 改固定值 + 横滚兜底（替代 ResizeObserver 动态算宽——flex minWidth 反撑场景首帧测量偏小，992px 可用只铺出 10px 格；窄容器不再压格）、统计容器 max-w-3xl→5xl；双端周一/周三/周五行标由「首尾均分」改为与网格行同构的 7 槽精确对位（旧法"周三"中心落在周四行中心，格小错位明显）；移动端格 12→14dp、图例色块同步格宽。
