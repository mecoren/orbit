//! paths — 云端同步文件路径常量与构造函数（v2 布局）
//!
//! 集中管理云同步的云端文件路径，避免路径字符串散落。
//!
//! ## v2 布局（2026-09-17 起）
//! 开发阶段云端无历史数据，直接切换到 v2 单清单 + 表级分桶布局，
//! **不保留** `.waitsync` / 裸文件名 / 旧全局清单的读取兼容（旧路径回退
//! 曾带来三/四段回退与每轮额外 HEAD/GET 请求，已整体移除）。
//!
//! ```text
//! {base_path}/
//! ├─ crypto/config                        # 加密元数据（无扩展名，跨设备分发 Key）
//! ├─ v2/manifest.orsync                   # 唯一真相源：epoch + 表分桶索引 + 墓碑水位线
//! ├─ v2/tables/{table}/{bucket:02}.orsync # 表级行分桶（内容寻址式差量上传单元）
//! ├─ v2/tombstones/{table}/{YYYY-MM}.orsync # 墓碑按本地时区月份分桶（可安全回收）
...
/// 云端清单路径（唯一真相源）
pub const MANIFEST_PATH: &str = "v2/manifest.orsync";
...
pub const MANIFEST_PREV_PATH: &str = "v2/manifest.prev.orsync";

/// 表分桶目录前缀
pub const TABLES_DIR: &str = "v2/tables";

/// 墓碑分桶目录前缀
pub const TOMBSTONES_DIR: &str = "v2/tombstones";

/// 附件目录前缀
pub const ASSETS_DIR: &str = "assets";

/// 构造表分桶路径：`v2/tables/{table}/{bucket:02}.orsync`
///
...
/// 构造墓碑分桶路径：`v2/tombstones/{table}/{YYYY-MM}.orsync`
///
...
mod tests {
    ...
    #[test]
    fn table_bucket_path_is_zero_padded_and_under_v2() {
        assert_eq!(
            table_bucket_path("todo_tasks", 7),
            "v2/tables/todo_tasks/07.orsync"
        );
        assert_eq!(
            table_bucket_path("todo_tasks", 63),
            "v2/tables/todo_tasks/63.orsync"
        );
    }
    ...
    fn tombstone_bucket_path_uses_month_key() {
        assert_eq!(
            ...
            "v2/tombstones/todo_tasks/2026-09.orsync"
        );
    }

    #[test]
    fn manifest_and_asset_paths_are_stable() {
        assert_eq!(MANIFEST_PATH, "v2/manifest.orsync");
    ...
    fn is_sync_payload_name_matches_orsync_only() {
        assert!(is_sync_payload_name("v2/manifest.orsync"));
