...
- 品牌设计（v5，现行）：**透明底 + AI 生图「圆环轨道」构图（加强版）**
  ...
  - 资产管线：最大连通域去散点 → 裁主体紧框 920×816 固化
    ...
    glow 仅边缘 23..44px 柔边）；`generate_icons.py` **短轴定标**——
    `fit_scale` 以 solid 短轴（=蓝色圆环外径 812，视觉主体）撑满画布
    86%（PAD=0.07），并夹逼 `ss/资产宽` 保证整幅资产不越出画布（尾梢
    零裁切；源图已紧框裁过、glow 余量≈0，环占 88.2% 为 clip-free 上
    限）；通知剪影复用同一 fit_scale 铺放后取 alpha>128 二值化纯白
    （pad=0.10，防状态栏内缩切 24px 档尾梢）。
  - 小尺寸特调（2026-09-08 起，任务栏模糊反馈；2026-09-18 起仅存
    alpha 档）：≤32px 走 SMALL_TIERS alpha 陡化（16/20: [160,235]→
    [0,255]；24: [150,240]；32: [140,245]）去抗锯齿灰雾；旧分档自带的
    PAD 几何放大维度（width 口径 0.05~0.08）已被全局短轴定标取代（环
    占画布 71%→86%），实测 16px 实蓝像素 55→58、不透明覆盖 ≥22%。
    ICO 改手写多槽容器（Pillow sizes 不支持
