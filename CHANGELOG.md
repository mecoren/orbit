...
- **测试**：Rust 9（当前年滚动窗/历史年完整年/available_years 回退/聚合七路）；桌面 vitest 189 全绿；移动 widget 3（总览+热力图渲染/色阶锚定 max≥4/空态）+ 全量 208 绿。门禁：cargo 391（m4 WebDAV 用例环境依赖失败为已知非回归）+ tsc 0 + flutter analyze 本批 0。
