...
| 竞品侧证据强度 | 公开认知，标「须真机验证」；Orbit 侧一律给文件:行号 |
...
| 侧滑完成/删除（滑出式动作面板） | `sub_list_screen.dart:1737-1775`（`flutter_slidable`） |
...
| CSV 导入迁移预设（含主流工具模板三档） | `settings_screen.dart:209-213` |
...
> 结论：**头部竞品待办主线的高频能力，移动端基本齐备**。差距集中在「核心已有但移动端未接线」与少量结构性空白。
...
| 移动端 | ❌ 未接线 | `settings_screen.dart:209-213` 导入预设缺 `ics` 档；`:219-220` `allowedExtensions: ['csv','txt']` |

- **用户价值**：从 Google Calendar / Outlook / Apple 日历等导出的 ICS 一次性迁入任务（VTODO）。
- **改造面**：**纯 Dart**（补 1 个预设 chip + 扩展名放行 + 卡片文案）；零 Rust、零 DDL、零同步面。
- 与竞品的关系：头部竞品支持 ICS 导入/订阅（**须真机验证**）；一次性导入属其导入能力子集。
...
- **用户价值**：头部竞品为秒级同步（**须真机验证**）；Orbit 双端现状为「编辑后约 5 秒推送」，此前移动端「手动触发为准」的对称性缺失已收口。
...
### A-4 移动端列表/看板/表格行不显示提醒徽标，信息密度低于桌面与头部竞品
...
- **用户价值**：头部竞品移动端行内可见标签/子任务进度/附件/提醒标记（**须真机验证**）；Orbit 需进入详情才能确认这些状态。
...
> 判定原则：移动端视觉蓝本是 wait-home（`docs/04`/`docs/05`），**不是对标竞品**。
> 因此「设计语言不同」不自动等于差距；只有「结构缺失 / 信息不全 / 触达不便」才计入。

| 维度 | Orbit 现状 | 竞品侧（须真机验证） | 结论 |
|---|---|---|---|
...
- **结构性空白只有四类**：清单文件夹分组、任务时长与时间块、子任务多层、日历订阅 —— 前三类是头部竞品的「厚度」，但都动 DDL 与同步面；建议按需而非按对标清单推进。
- **移动端相对桌面的不对称已收口**（2026-09-23：议程档 / 行内提醒徽标 / 密钥治理入口 /「修改后立即同步」四项全部落地，见 `docs/07 §五 #54–#58`）——这类改动零协议风险、可直接复用桌面既有实现，比相对竞品的差距更值得优先处理。
