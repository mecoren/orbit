...
- **移动端描述编辑弹层从 AlertDialog 改底部抽屉**:原 maxLines:5 固定 5 行长描述看不全,点遮罩即关丢草稿;改 70% 屏高上限的多行自适应输入(键盘避让、内容未变只关闭不写库)。保存语义(trim 空写 null)与原一致。docs/04 §3.4、docs/05 §4.3 规格同步。随 b7c4750/b2654ff 落地(代码随移除结束日期重构一并入库)。
