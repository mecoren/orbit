...
- **开机自启动（07 backlog #51）**：设置页新增「通用」分类与「开机自启动」开关，读写系统
  启动项（Windows `HKCU\...\Run` / macOS LaunchAgent plist / Linux XDG `.desktop`，由
  `tauri-plugin-autostart` 承担），**不做本地状态副本**——开关状态以系统为准（在系统
  启动项管理里禁用后会反映为关闭）。自启拉起的进程带 `--hidden`：照常启动（托盘 + 四个
  后台守护齐全）但**不弹主窗**，壳层同步接上隐藏回收链（WebView2 降档 + 超时销毁），
  常驻内存回落宿主档（~330MB → ~40MB），托盘/热键唤起走既有重建路径。
  已知口径：安装路径含空格时 Windows 注册项会被系统解析坏（插件底层拼注册值不加引号）；
  卸载前建议先关闭本开关。
